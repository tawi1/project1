...
public class FlexibleMusicManager : MonoBehaviour
{
    ...
    public IEnumerator LoadClipsFromResources(string path = "Music/")
    {
    ...
    void Init()
    {
        ...
        {
            StartCoroutine(LoadClipsFromResources());
        }

        Shuffle();
        Rewind();

        if (Prototype.NetworkLobby.SettingsController.GetMusic() == true)
            playOnAwake = true;
